...
#define mam_heap_will_overflow(heap, size) (heap->mem_size + mam__heap_correct_size(mam_checker_correct_size(size)) > heap->mem_capacity)
...
MAM_ALLOC__DECLR mam_int mam_heap_alloci(MamHeap* heap, mam_int size) {
	...
	MAM_ALLOC_ASSERT(!(heap->mem_size + size > heap->mem_capacity), "mam_alloc: failed to reallocate memory for heap");
...
MAM_ALLOC__DECLR mam_int mam_heap_realloci(MamHeap* heap, mam_int item, mam_int size) {
	...
	if(cur_block->size >= size) {
		...
		mam_int next_i = mam__block_get_next(cur_block, cur_i);
		Mam__Block* next_block = mam_ptr_add(Mam__Block, heap, next_i);
		if(next_block->free_pre) {
			//next block is free, use
		} else {
			mam_int new_item = mam_heap_alloci(heap, mam_checker_uncorrect_size(size));
			MAM_ALLOC_MEMCPY(mam_ptr_add(void, heap, new_item), mam_ptr_add(void, heap, item), cur_block->size);
			mam_heap_freei(heap, mam_checker_correct_item(item));
			return new_item;
		}
	}
}
...
MAM_ALLOC__DECLS mam_int mam_heap_lasti(MamHeap* heap) {
	if(heap->end_block) {
		return mam_checker_correct_item(heap->end_block + sizeof(Mam__Block));
	} else {
		return 0;
	}
}
...
MAM_ALLOC__DECLS mam_int mam_heap_prei(MamHeap* heap, mam_int item) {
	...
	mam_int pre_i = cur_block->pre;
	if(pre_i < cur_i) {
		return mam_checker_correct_item(pre_i + sizeof(Mam__Block));
	} else {
		return 0;
	}
}
